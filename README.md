...
# Objetivo
Será utilizada um banco de dados de alarmes de um sistema de telegestão baseado em dispositivos IoT semelhantes a relés utilizados em iluminação pública. Esses dispositivos IoT são chamados de LCU e a cidade em que o sistema de telegestão é aplicado possui 10.500 LCUs instaladas em luminárias LED nas grandes avenidas.
...
Os dados coletados foram dos sites Exati e Bright City que são sistemas de gestão operacionais contratados pela Concessionária de Iluminação Pública Conecta Aracaju, empresa em que trabalho atualmente.
...
Todas as perguntas criadas eram, na verdade, questões do nosso dia a dia na empresa que não conseguíamos muitas vezes responder. Somente as questões relacionadas ao dia atual podiam ser respondidas com facilidade e as outras dependiam que todos os dias fosse exportado um relatório em Excel do sistema, pois não existe um banco de dados nos sistemas contratados em que seja possível fazer consultas gerenciais.
Quando comecei a criar o MVP não imagina o quão podereso seria ter os dados bem organizados em uma estrutura de DW. Com os dados obtidos foi possível perceber padrões que antes nem sequer tínhamos noção que aconteciam.
...
Inclusive, em relação a pergunta de número 8, foi criado um relatório, baseado nos resultados da consulta, que foi enviado à empresa responsável pelo sistema de telegestão, que reconheceu que pode existir algum tipo de anomalia no dispotivo IoT.

Finalizando, todos os objetivos foram alcançados e os resultados foram bem maiores do que as expectativas iniciais.
